...
class Count extends React.Component {
    constructor(props) {
        ...
        this.state = {
            checked: false,
            arr0 : [{
                icon: (<span style={{fontSize: '30px', color: 'red'}}>{this.props.newProgramCount}</span>),
                text: '动态'
            }, {
                icon: (<span style={{fontSize: '30px', color: 'red'}}>{this.props.artistCount}</span>),
                text: '关注'
            }, {
                icon: (<span style={{fontSize: '30px', color: 'red'}}>{this.props.createdPlaylistCount}</span>),
                text: '粉丝'
            }, {
                icon: (<span style={{fontSize: '30px', color: 'red'}}>{this.props.newProgramCount}</span>),
                text: '编辑资料'
        ...
        this.data0 = this.state.arr0.map((v, i) => ({
            icon: v.icon,
            text: v.text
        }));
    }
            ...
            <div>
                {
                    this.props.token ?
                        (<div>
                            ...
                            <List>
                                < Item
                                    style={{background:'url('+this.props.userInfo.avatarUrl+')',backgroundSize:'100% 50px',boxShadow:'100% 50px 80px red inset'}}
                                    extra = {<i className={'iconfont icon-more2'} style={{fontSize: '15px'}}/>}
                                    align="top"
                                    thumb={(
                                        this.props.token ?
